...
function Auth (mysqlConnection: mysql.Connection) {
    ...
    router.use(cleanParams(mysqlConnection))

    /**
     * /api/auth
     */
    router.post('/', (req: express.Request, res: express.Response) => {
        mysqlConnection.query(`SELECT id, password FROM users WHERE phone = ${req.body?.phone}`, (error: mysql.MysqlError, results: any, fields: mysql.FieldInfo[]) => {
            if (error) {
    ...
    router.post('/register', (req: express.Request, res: express.Response) => {
        const validPass = req.body?.password.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/g);
        if (!validPass)
